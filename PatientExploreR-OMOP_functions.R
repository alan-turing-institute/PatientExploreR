...
standard_concepts <- data.table(domain_type = c("Measurement", "Condition", "Drug",
  "Observation", "Device", "Procedure"), concepts = c("LOINC,SNOMED,CPT4", "SNOMED",
  "RxNorm,CPT4,NDC", "SNOMED,CPT4,LOINC,HCPCS", "SNOMED,HCPCS", "SNOMED,CPT4,HCPCS"))
...
checkOMOPconnection <- function(driver, username, password, host, dbname, port) {
  ...
  status <- tryCatch({
    if (driver == "mysql") {
      drv <- dbDriver("MySQL")
      fullConnectString <- setConnectFunction(username, password, host, dbname,
        port)
      con <- eval(parse(text = fullConnectString))
    } else {
      con <- DatabaseConnector::connect(dbms = driver, server = host, user = username,
        password = password, schema = dbname, port = port)
...
checkOMOPtables <- function(driver, username, password, host, dbname, port) {
  ...
  necessaryTables <- c("concept", "concept_ancestor", "concept_relationship", "condition_occurrence",
...
getDemographics <- function() {
  ...
  ptDemo <- merge(ptDemo, dataOntology[domain_id == "Gender", c("concept_id", "concept_name")],
    by.x = "gender_concept_id", by.y = "concept_id", all.x = T)  # Gender
  names(ptDemo)[names(ptDemo) == "concept_name"] <- "Gender"  # rename column
  ptDemo <- markNAasUnknown(ptDemo, "Gender")

  ptDemo <- merge(ptDemo, dataOntology[domain_id == "Race", c("concept_id", "concept_name")],
    by.x = "race_concept_id", by.y = "concept_id", all.x = T)  # Race
  names(ptDemo)[names(ptDemo) == "concept_name"] <- "Race"  # rename column
  ptDemo <- markNAasUnknown(ptDemo, "Race")

  ptDemo <- merge(ptDemo, dataOntology[domain_id == "Ethnicity", c("concept_id",
    "concept_name")], by.x = "ethnicity_concept_id", by.y = "concept_id", all.x = T)  # Ethnicity
...
identifySynonyms <- function(codesFormatted) {
  synonymQuery <- paste0("SELECT concept_id_1, concept_id_2, relationship_id, invalid_reason FROM concept_relationship WHERE concept_id_1 IN (",
    codesFormatted, ");")
  ...
  synonymDataFiltered <- synonymData[(relationship_id == "Maps to") | (grepl("- RxNorm",
    relationship_id)) | (grepl("- SNOMED", relationship_id))]
...
identifyMappings <- function(synonymCodes) {
  ...
  mappingQuery <- paste0("SELECT ancestor_concept_id, descendant_concept_id FROM concept_ancestor A WHERE A.ancestor_concept_id IN (",
    synonymCodes, " );")
  mappingData <- sqlQuery(mappingQuery)
  mappingData <- data.table::data.table(mappingData)

  mappingDataInfo <- merge(mappingData, dataOntology, by.x = "descendant_concept_id",
    by.y = "concept_id")
...
identifyTablesMapped <- function(mappingDataInfo) {
  ...
  for (d in unique(standard_concepts$domain_type)) {
    ...
    mappingDataInfoFiltered <- mappingDataInfoFiltered[(grep(gsub(",", "|", standard_concepts[domain_type ==
      d, concepts]), vocabulary_id))]  # map to common concepts specifically used to the domain
...
identifyPatientsOR <- function(pts_condition, pts_observation, pts_measurement, pts_device,
  pts_drug, pts_procedure) {
...
identifyPatientsAND <- function(criteriaMapped, synonymDataFiltered, mappingDataInfo,
  ...
  synonymMapped <- merge(mappingDataInfo[, c("descendant_concept_id", "ancestor_concept_id",
    "mapped_vocabulary_id", "mapped_concept_name")], synonymDataFiltered[, c("concept_id_1",
    "concept_id_2")], by.x = "ancestor_concept_id", by.y = "concept_id_2", allow.cartesian = TRUE)
  synonymMapped <- synonymMapped[!duplicated(synonymMapped)]

  combinedMapped <- merge(synonymMapped, criteriaMapped, by.x = "concept_id_1",
    by.y = "concept_id", allow.cartesian = TRUE)
  combinedMapped <- combinedMapped[!duplicated(combinedMapped)]

  combinedDirect <- merge(mappingDataInfo, criteriaMapped, by.x = "ancestor_concept_id",
    by.y = "concept_id", allow.cartesian = TRUE)
  ...
  for (c in unique_codes) {
    ...
    code_map[[c]] <- c(code_map[[c]], combinedDirect[ancestor_concept_id %in%
      seed_codes]$descendant_concept_id)  # add in direct mapped descendants
  ...
  if (!is.null(pts_condition)) {
    ...
    for (c in condition_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_condition[condition_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
  ...
  if (!is.null(pts_observation)) {
    ...
    for (c in observation_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_observation[observation_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
  ...
  if (!is.null(pts_measurement)) {
    ...
    for (c in measurement_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_measurement[measurement_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
  ...
  if (!is.null(pts_device)) {
    ...
    for (c in device_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_device[device_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
  ...
  if (!is.null(pts_drug)) {
    ...
    for (c in drug_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_drug[drug_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
  ...
  if (!is.null(pts_procedure)) {
    ...
    for (c in procedure_codes) {
      patient_list[[c]] <- union(patient_list[[c]], pts_procedure[procedure_concept_id %in%
        code_map[[c]]]$person_id)
    }
  }
...
generate_pt_summary <- function(pt_data) {
  ...
  earliest_date <- as.Date(encounters$visit_start_date[order(encounters$visit_start_date,
    decreasing = F)[1]])
  recent_date <- as.Date(encounters$visit_start_date[order(encounters$visit_start_date,
    decreasing = T)[1]])
  ...
  str5a <- paste0("<strong># Outpatient encounters:</strong> ", nrow(deduped_encounters[which(deduped_encounters$visit_concept ==
    "Outpatient Visit"), ]))
  str6a <- paste0("<strong># Inpatient encounters:</strong> ", nrow(deduped_encounters[which(deduped_encounters$Encounter_Is_Inpatient ==
    "Inpatient Visit"), ]))
  ...
  strsb <- c(str1b, str2b, str3b, str4b, str5b, str6b, str7b, str8b, str9b, str10b,
    str11b, str12b)
...
generate_pt_report <- function(pt_data) {
  ...
  master_report <- data.table(Date = as.Date(character()), Date_end = character(),
    Type = character(), Event = character(), Value = character())
  ...
  ## format observations
  observations <- observations_original[, c("observation_date", "observation_concept_name",
    "value_as_number")]
  ...
  observations <- observations[, c("observation_date", "Date_end", "Type", "observation_concept_name",
    "value_as_number")]
  colnames(observations) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format conditions
  conditions <- conditions_original[, c("condition_start_date", "condition_end_date",
    "condition_concept_name", "condition_source_value")]
  ...
  conditions <- conditions[, c("condition_start_date", "condition_end_date", "Type",
    "condition_concept_name", "condition_source_value")]
  colnames(conditions) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format procedures
  procedures <- procedures_original[, c("procedure_date", "procedure_concept_name",
  ...
  procedures <- procedures[, c("procedure_date", "Date_end", "Type", "procedure_concept_name",
    "procedure_source_value")]
  colnames(procedures) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format Medications
  medications <- medications_original[, c("drug_exposure_start_date", "drug_exposure_end_date",
    "medication_concept_name", "dose_unit_source_value")]
  ...
  medications <- medications[, c("drug_exposure_start_date", "drug_exposure_end_date",
    "Type", "medication_concept_name", "dose_unit_source_value")]
  colnames(medications) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format Measurements
  measurements <- measurements_original[, c("measurement_date", "measurement_concept_name",
    "value_as_number")]
  ...
  measurements <- measurements[, c("measurement_date", "Date_end", "Type", "measurement_concept_name",
    "value_as_number")]
  colnames(measurements) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format Devices
  devices <- devices_original[, c("device_exposure_start_date", "device_exposure_end_date",
    "device_concept_name", "device_source_value")]
  devices$Type <- "Device"
  ...
  devices <- devices[, c("device_exposure_start_date", "device_exposure_end_date",
    "Type", "device_concept_name", "device_source_value")]
  colnames(devices) <- c("Date", "Date_end", "Type", "Event", "Value")


  ## rbind all data modalities together
  master_report <- rbind(master_report, observations, conditions, procedures, medications,
...
format_multiplex_timeline <- function(pt_data_report) {
  ...
  multiplex_timeline <- multiplex_timeline[, c("id", "Event", "Date", "Date_end",
    "Type", "type", "Value")]  # keep Value to display when clicked
  colnames(multiplex_timeline) <- c("id", "content", "start", "end", "group", "type",
    "Value")
...
get_all_pt_data <- function(pt_id) {
  ...
  return(list(Encounters = ptEncs, Observations = ptObsData, Conditions = ptCondData,
...
getEncounters <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, visit_occurrence_id, visit_concept_id, visit_start_date, visit_end_date, visit_source_concept_id, visit_source_value, admitting_source_concept_id, discharge_to_concept_id FROM visit_occurrence WHERE person_id = ",
    pt_id)
  ...
  ptEncs <- merge(ptEncs, dataOntology[, c("concept_id", "concept_name")], by.x = "visit_concept_id",
    by.y = "concept_id", all.x = TRUE)
  names(ptEncs)[names(ptEncs) == "concept_name"] <- "visit_concept"  # rename column
  ptEncs <- ptEncs[, -"visit_concept_id"]
  ptEncs <- merge(ptEncs, dataOntology[, c("concept_id", "concept_name")], by.x = "visit_source_concept_id",
    by.y = "concept_id", all.x = TRUE)
  names(ptEncs)[names(ptEncs) == "concept_name"] <- "visit_source_concept"  # rename column
  ptEncs <- ptEncs[, -"visit_source_concept_id"]
  ptEncs <- merge(ptEncs, dataOntology[, c("concept_id", "concept_name")], by.x = "admitting_source_concept_id",
    by.y = "concept_id", all.x = TRUE)
  names(ptEncs)[names(ptEncs) == "concept_name"] <- "admitting_concept"  # rename column
  ptEncs <- ptEncs[, -"admitting_source_concept_id"]
  ptEncs <- merge(ptEncs, dataOntology[, c("concept_id", "concept_name")], by.x = "discharge_to_concept_id",
    by.y = "concept_id", all.x = TRUE)
...
getObservations <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, observation_concept_id, observation_source_concept_id, observation_date, observation_type_concept_id, value_as_number, value_as_string, value_as_concept_id, visit_occurrence_id, observation_source_value, unit_source_value FROM observation WHERE person_id = ",
    pt_id)
  ...
  ptObsData <- merge(ptObsData, observationTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "observation_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptObsData <- merge(ptObsData, observationTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "observation_source_concept_id",
    by.y = "concept_id", all.x = TRUE)
  ...
  # format metadata
  ptObsData <- merge(ptObsData, dataOntology[, c("concept_id", "concept_name")],
...
getConditions <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, condition_concept_id, condition_start_date, condition_end_date, visit_occurrence_id, condition_type_concept_id, condition_source_value, condition_source_concept_id, condition_status_concept_id FROM condition_occurrence WHERE person_id = ",
    pt_id)
  ...
  # format clinical data
  ptCondData <- merge(ptCondData, conditionTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "condition_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptCondData <- merge(ptCondData, conditionTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "condition_source_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  # format metadatadata
  ptCondData <- merge(ptCondData, dataOntology[, c("concept_id", "concept_name")],
...
getProcedures <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, procedure_concept_id, procedure_date, quantity, visit_occurrence_id, procedure_type_concept_id, procedure_source_value, procedure_source_concept_id  FROM procedure_occurrence WHERE person_id = ",
    pt_id)
  ...
  # format clinical data
  ptProcData <- merge(ptProcData, procedureTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "procedure_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptProcData <- merge(ptProcData, procedureTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "procedure_source_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  # format metadata
  ptProcData <- merge(ptProcData, dataOntology[, c("concept_id", "concept_name")],
...
getMedications <- function(pt_id) {
  ...
  # format clinical data
  ptsMedsData <- merge(ptsMedsData, medicationTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "drug_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptsMedsData <- merge(ptsMedsData, medicationTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "drug_source_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  # format metadata
  ptsMedsData <- merge(ptsMedsData, dataOntology[, c("concept_id", "concept_name")],
...
getMeasurements <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, measurement_concept_id, measurement_date, measurement_type_concept_id, value_as_number, value_as_concept_id, unit_concept_id, visit_occurrence_id, measurement_source_value, measurement_source_concept_id FROM measurement WHERE person_id = ",
    pt_id)
  ...
  # format clinical data
  ptMeasData <- merge(ptMeasData, measurementTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "measurement_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptMeasData <- merge(ptMeasData, measurementTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "measurement_source_concept_id",
    by.y = "concept_id", all.x = TRUE)
  ...
  # format metadata
  ptMeasData <- merge(ptMeasData, dataOntology[, c("concept_id", "concept_name")],
...
getDevices <- function(pt_id) {
  ...
  queryStatement <- paste0("SELECT person_id, device_concept_id, device_exposure_start_date, device_exposure_end_date, device_type_concept_id, device_source_value, visit_occurrence_id, device_source_concept_id FROM device_exposure WHERE person_id = ",
    pt_id)
  ...
  # format clinical data
  ptDeviceData <- merge(ptDeviceData, deviceTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "device_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  ptDeviceData <- merge(ptDeviceData, deviceTableOntology[, c("concept_id", "vocabulary_id",
    "concept_code", "concept_name")], by.x = "device_source_concept_id", by.y = "concept_id",
    all.x = TRUE)
  ...
  # format metadata
  ptDeviceData <- merge(ptDeviceData, dataOntology[, c("concept_id", "concept_name")],
...
findPatients <- function(selected_terms, func_type, search_strat) {
  ...
  if (search_strat == "direct") {
    ...
    synonymData <- merge(synonymDataFiltered[, "concept_id_2"], dataOntology[,
      c("concept_id", "domain_id", "vocabulary_id")], by.x = "concept_id_2",
      by.y = "concept_id")
    colnames(synonymData) <- c("concept_id", "domain_id", "vocabulary_id")
    synonymCodes <- paste(union(dataCriteriaMapped$concept_id, synonymDataFiltered$concept_id_2),
      collapse = ",")  ## adds original codes into ancestor query (b/c of scenarios with ATC))

    # get descendents
    mappingDataInfo <- identifyMappings(synonymCodes)
    mappingData <- mappingDataInfo[, c("descendant_concept_id", "domain_id",
      "vocabulary_id")]
    colnames(mappingData) <- c("concept_id", "domain_id", "vocabulary_id")

    conceptsCombined <- rbind(dataCriteriaMapped[, c("concept_id", "domain_id",
      "vocabulary_id")], synonymData)
  ...
  if (func_type == "or") {
    patient_list <- identifyPatientsOR(pts_condition, pts_observation, pts_measurement,
      pts_device, pts_drug, pts_procedure)
  } else if (func_type == "and") {
    patient_list <- identifyPatientsAND(dataCriteriaMapped, synonymDataFiltered,
      mappingDataInfo, pts_condition, pts_observation, pts_measurement, pts_device,
      pts_drug, pts_procedure)
  }
...
searchCondition <- function(useSource, codes) {
  conditionQuery <- paste0("SELECT person_id, condition_concept_id FROM condition_occurrence WHERE condition",
...
searchObservation <- function(useSource, codes) {
  observationQuery <- paste0("SELECT person_id, observation_concept_id FROM observation WHERE observation",
    useSource, "_concept_id IN (", codes, ") ")
...
searchMeasurement <- function(useSource, codes) {
  measurementQuery <- paste0("SELECT person_id, measurement_concept_id FROM measurement WHERE measurement",
    useSource, "_concept_id IN (", codes, ") ")
...
searchDrug <- function(useSource, codes) {
  drugQuery <- paste0("SELECT person_id, drug_concept_id FROM drug_exposure WHERE drug",
    useSource, "_concept_id IN (", codes, ") ")
...
searchDevice <- function(useSource, codes) {
  deviceQuery <- paste0("SELECT person_id, device_concept_id FROM device_exposure WHERE device",
    useSource, "_concept_id IN (", codes, ") ")
...
searchProcedure <- function(useSource, codes) {
  procedureQuery <- paste0("SELECT person_id, procedure_concept_id FROM procedure_occurrence WHERE procedure",
    useSource, "_concept_id IN (", codes, ") ")
