...
standard_concepts <- data.table(
  domain_type = c("Measurement", "Condition", "Drug", "Observation", "Device", "Procedure"),
  concepts = c(
    "LOINC,SNOMED,CPT4", "SNOMED", "RxNorm,CPT4,NDC", "SNOMED,CPT4,LOINC,HCPCS",
    "SNOMED,HCPCS", "SNOMED,CPT4,HCPCS"
  )
)
...
getDBConnection <- function() {
  con <- DatabaseConnector::connect(
    dbms = Sys.getenv("EXPLORER_DBMS"),
    user = Sys.getenv("EXPLORER_USERNAME"),
    password = Sys.getenv("EXPLORER_PASSWORD"),
    server = paste0(
      Sys.getenv("EXPLORER_HOSTNAME"),
      "/", Sys.getenv("EXPLORER_DBNAME")
    ),
    port = Sys.getenv("EXPLORER_PORT"),
    pathToDriver = Sys.getenv("EXPLORER_PATH_TO_DRIVER")
  )
  return(con)
}
...
checkOMOPtables <- function() {
  ...
  necessaryTables <- c(
    ...
    "death", "device_exposure", "drug_exposure", "measurement", "observation",
  ...
  for (tbls in necessaryTables) {
    ...
    if (!tbls %in% foundTablesData) {
      # check if table exists
      missingTables <- c(missingTables, tbls)
    } else {
      # check if any data in found table
...
make_data_ontology <- function() {
  if (file.exists(
    paste0(
      getOption("currentPath"),
      "dataOntology.rds"
    )
  )) {
    # if Data Ontology exists, load it
    dataOntology <- readRDS(
      paste0(
        getOption("currentPath"),
        "dataOntology.rds"
      )
...
# Demographic data pre-load demographic data for all patients to save in memory
# to map to cohort from searches

getDemographics <- function() {
  # patient list will restrict search
  ...
  ptDemo <- merge(
    ptDemo, dataOntology[domain_id == "Gender", c("concept_id", "concept_name")],
    by.x = "gender_concept_id", by.y = "concept_id", all.x = T
  )  # Gender
  names(ptDemo)[names(ptDemo) ==
    "concept_name"] <- "Gender"  # rename column
  ptDemo <- markNAasUnknown(ptDemo, "Gender")

  ptDemo <- merge(
    ptDemo, dataOntology[domain_id == "Race", c("concept_id", "concept_name")],
    by.x = "race_concept_id", by.y = "concept_id", all.x = T
  )  # Race
  names(ptDemo)[names(ptDemo) ==
    "concept_name"] <- "Race"  # rename column
  ptDemo <- markNAasUnknown(ptDemo, "Race")

  ptDemo <- merge(
    ptDemo, dataOntology[domain_id == "Ethnicity", c("concept_id", "concept_name")],
    by.x = "ethnicity_concept_id", by.y = "concept_id", all.x = T
  )  # Ethnicity
  names(ptDemo)[names(ptDemo) ==
    "concept_name"] <- "Ethnicity"  # rename column
  ...
  # mark Alive/Deceased
  ptDemo$Status <- ifelse(
    is.na(ptDemo$death_date),
    "Alive", "Deceased"
  )
...
unpackAndMap <- function(vocab_term_list) {
  vocabularies <- str_split(vocab_term_list, ":") %>%
    map_chr(`[`, 1)
  codes <- str_split(vocab_term_list, ":") %>%
    map_chr(`[`, 2)
...
identifySynonyms <- function(codesFormatted) {
  synonymQuery <- paste0(
    "SELECT concept_id_1, concept_id_2, relationship_id, invalid_reason FROM concept_relationship WHERE concept_id_1 IN (",
    codesFormatted, ");"
  )
  ...
  synonymDataFiltered <- synonymData[(relationship_id == "Maps to") | (grepl("- RxNorm", relationship_id)) |
...
# for 'Mapped' straegy; map input concept codes (from common ontology) to
# common ontology descendants
identifyMappings <- function(synonymCodes) {

  mappingQuery <- paste0(
    "SELECT ancestor_concept_id, descendant_concept_id FROM concept_ancestor A WHERE A.ancestor_concept_id IN (",
    synonymCodes, " );"
  )
...
identifyTablesDirect <- function(criteriaTable) {
  ...
  for (d in unique(standard_concepts$domain_type)) {
    # scan through all domain types
...
identifyTablesMapped <- function(mappingDataInfo) {
  ...
  for (d in unique(standard_concepts$domain_type)) {
    ...
    mappingDataInfoFiltered <- mappingDataInfoFiltered[(grep(
      gsub(",", "|", standard_concepts[domain_type == d, concepts]),
      vocabulary_id
    ))]  # map to common concepts specifically used to the domain
...
### identifyPatients based on function function = OR (union)
identifyPatientsOR <- function(
  pts_condition, pts_observation, pts_measurement, pts_device, pts_drug, pts_procedure
) {
...
# function = AND (intersect) To identify overlapping patients, we have to
# backmap the descendant terms to the original concepts
identifyPatientsAND <- function(
  criteriaMapped, synonymDataFiltered, mappingDataInfo, pts_condition, pts_observation,
  pts_measurement, pts_device, pts_drug, pts_procedure
) {

  names(mappingDataInfo)[names(mappingDataInfo) ==
    "vocabulary_id"] <- "mapped_vocabulary_id"
  names(mappingDataInfo)[names(mappingDataInfo) ==
    "concept_name"] <- "mapped_concept_name"

  synonymMapped <- merge(
    mappingDataInfo[, c(
      "descendant_concept_id", "ancestor_concept_id", "mapped_vocabulary_id",
      "mapped_concept_name"
    )],
    synonymDataFiltered[, c("concept_id_1", "concept_id_2")],
    by.x = "ancestor_concept_id", by.y = "concept_id_2", allow.cartesian = TRUE
  )
  synonymMapped <- synonymMapped[!duplicated(synonymMapped)]

  combinedMapped <- merge(
    synonymMapped, criteriaMapped, by.x = "concept_id_1", by.y = "concept_id",
    allow.cartesian = TRUE
  )
  combinedMapped <- combinedMapped[!duplicated(combinedMapped)]

  combinedDirect <- merge(
    mappingDataInfo, criteriaMapped, by.x = "ancestor_concept_id", by.y = "concept_id",
    allow.cartesian = TRUE
  )
  combinedDirect <- combinedDirect[!duplicated(combinedDirect)]


  ### derive patient list by concept_codes create code dictionary per original
  ### concept input initializepatient_list
  ...
  for (c in unique_codes) {
    ...
    code_map[[c]] <- c(
      code_map[[c]], combinedDirect[ancestor_concept_id %in% seed_codes]$descendant_concept_id
    )  # add in direct mapped descendants
    code_map[[c]] <- c(
      code_map[[c]], combinedMapped[concept_id_1 %in% seed_codes]$descendant_concept_id
    )  # add in synonym codes and descendants
  ...
  if (!is.null(pts_condition)) {
    ...
    for (c in condition_codes) {
      patient_list[[c]] <- union(
        patient_list[[c]], pts_condition[condition_concept_id %in% code_map[[c]]]$person_id
      )
    }
  }
  ...
  if (!is.null(pts_observation)) {
    ...
    for (c in observation_codes) {
      patient_list[[c]] <- union(
        patient_list[[c]], pts_observation[observation_concept_id %in% code_map[[c]]]$person_id
      )
    }
  }
  ...
  if (!is.null(pts_measurement)) {
    ...
    for (c in measurement_codes) {
      patient_list[[c]] <- union(
        patient_list[[c]], pts_measurement[measurement_concept_id %in% code_map[[c]]]$person_id
      )
    }
  }
  ...
  if (!is.null(pts_procedure)) {
    ...
    for (c in procedure_codes) {
      patient_list[[c]] <- union(
        patient_list[[c]], pts_procedure[procedure_concept_id %in% code_map[[c]]]$person_id
      )
    }
  }
...
generate_pt_background <- function(pt_background) {
  ...
  if (!is.na(pt_background$death_date)) {
    age_of_death <- as.numeric(year(as.Date(pt_background$death_date))) -
      as.numeric(pt_background$year_of_birth)
  } else {
    age_of_death <- NA
  }
...
generate_pt_summary <- function(pt_data) {
  ...
  earliest_date <- as.Date(
    encounters$visit_start_date[order(encounters$visit_start_date, decreasing = F)[1]]
  )
  recent_date <- as.Date(
    encounters$visit_start_date[order(encounters$visit_start_date, decreasing = T)[1]]
  )

  str1a <- paste0("<strong>Earliest encounter:</strong> ", earliest_date)
  str2a <- paste0("<strong>Most recent encounter:</strong> ", recent_date)
  str3a <- paste0(
    "<strong># unique encounter types:</strong> ", length(unique(deduped_encounters$visit_concept))
  )
  str4a <- paste0("<strong># Encounters:</strong> ", nrow(deduped_encounters))
  str5a <- paste0(
    "<strong># Outpatient encounters:</strong> ", nrow(
      deduped_encounters[which(deduped_encounters$visit_concept == "Outpatient Visit"),
        ]
    )
  )
  str6a <- paste0(
    "<strong># Inpatient encounters:</strong> ", nrow(
      deduped_encounters[which(deduped_encounters$Encounter_Is_Inpatient == "Inpatient Visit"),
        ]
    )
  )
  ...
  str2b <- paste0(
    "<strong># unique observation concepts:</strong> ", length(
      unique(
        deduped_observations[!is.na(observation_concept_name)]$observation_concept_name
      )
    )
  )
  str3b <- paste0("<strong># conditions:</strong> ", nrow(deduped_conditions))
  str4b <- paste0(
    "<strong># unique condition concepts:</strong> ", length(
      unique(deduped_conditions[!is.na(condition_concept_name)]$condition_concept_name)
    )
  )
  str5b <- paste0("<strong># procedures:</strong> ", nrow(deduped_procedures))
  str6b <- paste0(
    "<strong># unique procedure concepts:</strong> ", length(
      unique(deduped_procedures[!is.na(procedure_concept_name)]$procedure_concept_name)
    )
  )
  str7b <- paste0("<strong># medication prescriptions:</strong> ", nrow(deduped_medications))
  str8b <- paste0(
    "<strong># unique medication concepts:</strong> ", length(
      unique(deduped_medications[!is.na(medication_concept_name)]$medication_concept_name)
    )
  )
  str9b <- paste0("<strong># measurements:</strong> ", nrow(deduped_measurements))
  str10b <- paste0(
    "<strong># unique measurement concepts:</strong> ", length(
      unique(
        deduped_measurements[!is.na(measurement_concept_name)]$measurement_concept_name
      )
    )
  )
  str11b <- paste0("<strong># devices:</strong> ", nrow(deduped_devices))
  str12b <- paste0(
    "<strong># unique device concepts:</strong> ", length(unique(deduped_devices[!is.na(device_concept_name)]$device_concept_name))
  )
  strsb <- c(
    str1b, str2b, str3b, str4b, str5b, str6b, str7b, str8b, str9b, str10b, str11b,
    str12b
  )
...
generate_pt_report <- function(pt_data) {
  ...
  observations <- observations[, c(
    "observation_date", "Date_end", "Type", "observation_concept_name", "value_as_number"
  )]
  colnames(observations) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format conditions
  conditions <- conditions_original[, c(
    "condition_start_date", "condition_end_date", "condition_concept_name", "condition_source_value"
  )]
  ...
  conditions <- conditions[, c(
    "condition_start_date", "condition_end_date", "Type", "condition_concept_name",
  ...
  procedures <- procedures[, c(
    "procedure_date", "Date_end", "Type", "procedure_concept_name", "procedure_source_value"
  )]
  colnames(procedures) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format Medications
  medications <- medications_original[, c(
    "drug_exposure_start_date", "drug_exposure_end_date", "medication_concept_name",
  ...
  medications <- medications[, c(
    "drug_exposure_start_date", "drug_exposure_end_date", "Type", "medication_concept_name",
    "dose_unit_source_value"
  )]
  ...
  measurements <- measurements[, c(
    "measurement_date", "Date_end", "Type", "measurement_concept_name", "value_as_number"
  )]
  colnames(measurements) <- c("Date", "Date_end", "Type", "Event", "Value")

  ## format Devices
  devices <- devices_original[, c(
    "device_exposure_start_date", "device_exposure_end_date", "device_concept_name",
    "device_source_value"
  )]
  devices$Type <- "Device"
  ...
  devices <- devices[, c(
    "device_exposure_start_date", "device_exposure_end_date", "Type", "device_concept_name",
    "device_source_value"
  )]
  ...
  master_report <- rbind(
    master_report, observations, conditions, procedures, medications, measurements,
    devices
  )
  ...
  master_report$Date_end <- as.Date(
    as.character(master_report$Date_end),
    format = "%Y-%m-%d"
  )
...
# Wrapper for domain-specific getData functions (e.g., getObservations).
# Produces a list of tables for all relevant domains.
get_all_pt_data <- function(pt_id) {
  ...
  return(
    list(
      Encounters = ptEncs, Observations = ptObsData, Conditions = ptCondData,
      Procedures = ptProcData, Medications = ptsMedsData, Measurements = ptMeasData,
...
getEncounters <- function(pt_id) {
  ...
  ptEncs <- merge(
    ...
    by.x = "visit_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptEncs)[names(ptEncs) ==
    "concept_name"] <- "visit_concept"  # rename column
  ptEncs <- ptEncs[, -"visit_concept_id"]
  ptEncs <- merge(
    ptEncs, dataOntology[, c("concept_id", "concept_name")],
    by.x = "visit_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptEncs)[names(ptEncs) ==
    "concept_name"] <- "visit_source_concept"  # rename column
...
getObservations <- function(pt_id) {
  ...
  queryStatement <- paste0(
    "SELECT person_id, observation_concept_id, observation_source_concept_id, observation_date, observation_type_concept_id, value_as_number, value_as_string, value_as_concept_id, visit_occurrence_id, observation_source_value, unit_source_value FROM observation WHERE person_id = ",
    pt_id
  )
  ...
  ptObsData <- merge(
    ptObsData, observationTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "observation_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptObsData)[names(ptObsData) ==
    "concept_code"] <- "observation_concept_code"  # rename column
  names(ptObsData)[names(ptObsData) ==
    "concept_name"] <- "observation_concept_name"  # rename column
  names(ptObsData)[names(ptObsData) ==
    "vocabulary_id"] <- "observation_concept_vocabulary"  # rename column
  ptObsData <- ptObsData[, -"observation_concept_id"]

  ptObsData <- merge(
    ptObsData, observationTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "observation_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptObsData)[names(ptObsData) ==
    "concept_code"] <- "observation_source_code"  # rename column
  names(ptObsData)[names(ptObsData) ==
    "concept_name"] <- "observation_source_name"  # rename column
  names(ptObsData)[names(ptObsData) ==
    "vocabulary_id"] <- "observation_source_vocabulary"  # rename column
  ptObsData <- ptObsData[, -"observation_source_concept_id"]

  # format metadata
  ptObsData <- merge(
    ptObsData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "observation_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptObsData)[names(ptObsData) ==
    "concept_name"] <- "observation_type"  # rename column
  ptObsData <- ptObsData[, -"observation_type_concept_id"]

  ptObsData <- merge(
    ptObsData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "value_as_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptObsData)[names(ptObsData) ==
    "concept_name"] <- "value_concept"  # rename column
...
getConditions <- function(pt_id) {
  ...
  queryStatement <- paste0(
    "SELECT person_id, condition_concept_id, condition_start_date, condition_end_date, visit_occurrence_id, condition_type_concept_id, condition_source_value, condition_source_concept_id, condition_status_concept_id FROM condition_occurrence WHERE person_id = ",
    pt_id
  )
  ...
  # format clinical data
  ptCondData <- merge(
    ptCondData, conditionTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "condition_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptCondData)[names(ptCondData) ==
    "concept_code"] <- "condition_concept_code"  # rename column
  names(ptCondData)[names(ptCondData) ==
    "concept_name"] <- "condition_concept_name"  # rename column
  names(ptCondData)[names(ptCondData) ==
    "vocabulary_id"] <- "condition_concept_vocabulary"  # rename column
  ptCondData <- ptCondData[, -"condition_concept_id"]

  ptCondData <- merge(
    ptCondData, conditionTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "condition_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptCondData)[names(ptCondData) ==
    "concept_code"] <- "condition_source_code"  # rename column
  names(ptCondData)[names(ptCondData) ==
    "concept_name"] <- "condition_source_name"  # rename column
  names(ptCondData)[names(ptCondData) ==
    "vocabulary_id"] <- "condition_source_vocabulary"  # rename column
  ptCondData <- ptCondData[, -"condition_source_concept_id"]

  # format metadatadata
  ptCondData <- merge(
    ptCondData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "condition_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptCondData)[names(ptCondData) ==
    "concept_name"] <- "condition_type"  # rename column
  ptCondData <- ptCondData[, -"condition_type_concept_id"]
  ptCondData <- merge(
    ptCondData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "condition_status_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptCondData)[names(ptCondData) ==
    "concept_name"] <- "condition_status_type"  # rename column
...
getProcedures <- function(pt_id) {
  ...
  queryStatement <- paste0(
    "SELECT person_id, procedure_concept_id, procedure_date, quantity, visit_occurrence_id, procedure_type_concept_id, procedure_source_value, procedure_source_concept_id  FROM procedure_occurrence WHERE person_id = ",
    pt_id
  )
  ...
  # format clinical data
  ptProcData <- merge(
    ptProcData, procedureTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "procedure_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptProcData)[names(ptProcData) ==
    "concept_code"] <- "procedure_concept_code"  # rename column
  names(ptProcData)[names(ptProcData) ==
    "concept_name"] <- "procedure_concept_name"  # rename column
  names(ptProcData)[names(ptProcData) ==
    "vocabulary_id"] <- "procedure_concept_vocabulary"  # rename column
  ptProcData <- ptProcData[, -"procedure_concept_id"]

  ptProcData <- merge(
    ptProcData, procedureTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "procedure_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptProcData)[names(ptProcData) ==
    "concept_code"] <- "procedure_source_code"  # rename column
  names(ptProcData)[names(ptProcData) ==
    "concept_name"] <- "procedure_source_name"  # rename column
  names(ptProcData)[names(ptProcData) ==
    "vocabulary_id"] <- "procedure_source_vocabulary"  # rename column
  ptProcData <- ptProcData[, -"procedure_source_concept_id"]

  # format metadata
  ptProcData <- merge(
    ptProcData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "procedure_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptProcData)[names(ptProcData) ==
    "concept_name"] <- "procedure_type"  # rename column
...
getMedications <- function(pt_id) {
  ...
  # format clinical data
  ptsMedsData <- merge(
    ptsMedsData, medicationTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "drug_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_code"] <- "medication_concept_code"  # rename column
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_name"] <- "medication_concept_name"  # rename column
  names(ptsMedsData)[names(ptsMedsData) ==
    "vocabulary_id"] <- "medication_concept_vocabulary"  # rename column
  ptsMedsData <- ptsMedsData[, -"drug_concept_id"]

  ptsMedsData <- merge(
    ptsMedsData, medicationTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "drug_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_code"] <- "medication_source_code"  # rename column
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_name"] <- "medication_source_name"  # rename column
  names(ptsMedsData)[names(ptsMedsData) ==
    "vocabulary_id"] <- "medication_source_vocabulary"  # rename column
  ptsMedsData <- ptsMedsData[, -"drug_source_concept_id"]

  # format metadata
  ptsMedsData <- merge(
    ptsMedsData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "drug_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_name"] <- "drug_type"  # rename column
  ptsMedsData <- ptsMedsData[, -"drug_type_concept_id"]
  ptsMedsData <- merge(
    ptsMedsData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "route_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptsMedsData)[names(ptsMedsData) ==
    "concept_name"] <- "route_concept"  # rename column
...
getMeasurements <- function(pt_id) {
  ...
  queryStatement <- paste0(
    "SELECT person_id, measurement_concept_id, measurement_date, measurement_type_concept_id, value_as_number, value_as_concept_id, unit_concept_id, visit_occurrence_id, measurement_source_value, measurement_source_concept_id FROM measurement WHERE person_id = ",
    pt_id
  )
  ...
  # format clinical data
  ptMeasData <- merge(
    ptMeasData, measurementTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "measurement_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptMeasData)[names(ptMeasData) ==
    "concept_code"] <- "measurement_concept_code"  # rename column
  names(ptMeasData)[names(ptMeasData) ==
    "concept_name"] <- "measurement_concept_name"  # rename column
  names(ptMeasData)[names(ptMeasData) ==
    "vocabulary_id"] <- "measurement_concept_vocabulary"  # rename column
  ptMeasData <- ptMeasData[, -"measurement_concept_id"]

  ptMeasData <- merge(
    ptMeasData, measurementTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "measurement_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptMeasData)[names(ptMeasData) ==
    "concept_code"] <- "measurement_source_code"  # rename column
  names(ptMeasData)[names(ptMeasData) ==
    "concept_name"] <- "measurement_source_name"  # rename column
  names(ptMeasData)[names(ptMeasData) ==
    "vocabulary_id"] <- "measurement_source_vocabulary"  # rename column
  ptMeasData <- ptMeasData[, -"measurement_source_concept_id"]

  # format metadata
  ptMeasData <- merge(
    ptMeasData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "measurement_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptMeasData)[names(ptMeasData) ==
    "concept_name"] <- "measurement_type"  # rename column
  ...
  ptMeasData <- merge(
    ptMeasData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "unit_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptMeasData)[names(ptMeasData) ==
    "concept_name"] <- "unit_concept"  # rename column
...
getDevices <- function(pt_id) {
  ...
  queryStatement <- paste0(
    "SELECT person_id, device_concept_id, device_exposure_start_date, device_exposure_end_date, device_type_concept_id, device_source_value, visit_occurrence_id, device_source_concept_id FROM device_exposure WHERE person_id = ",
    pt_id
  )
  ...
  # format clinical data
  ptDeviceData <- merge(
    ptDeviceData, deviceTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "device_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptDeviceData)[names(ptDeviceData) ==
    "concept_code"] <- "device_concept_code"  # rename column
  names(ptDeviceData)[names(ptDeviceData) ==
    "concept_name"] <- "device_concept_name"  # rename column
  names(ptDeviceData)[names(ptDeviceData) ==
    "vocabulary_id"] <- "device_concept_vocabulary"  # rename column
  ptDeviceData <- ptDeviceData[, -"device_concept_id"]

  ptDeviceData <- merge(
    ptDeviceData, deviceTableOntology[, c("concept_id", "vocabulary_id", "concept_code", "concept_name")],
    by.x = "device_source_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptDeviceData)[names(ptDeviceData) ==
    "concept_code"] <- "device_source_code"  # rename column
  names(ptDeviceData)[names(ptDeviceData) ==
    "concept_name"] <- "device_source_name"  # rename column
  names(ptDeviceData)[names(ptDeviceData) ==
    "vocabulary_id"] <- "device_source_vocabulary"  # rename column
  ptDeviceData <- ptDeviceData[, -"device_source_concept_id"]

  # format metadata
  ptDeviceData <- merge(
    ptDeviceData, dataOntology[, c("concept_id", "concept_name")],
    by.x = "device_type_concept_id", by.y = "concept_id", all.x = TRUE
  )
  names(ptDeviceData)[names(ptDeviceData) ==
    "concept_name"] <- "device_type"  # rename column
...
findPatients <- function(selected_terms, func_type, search_strat) {
  ...
  if (search_strat == "direct") {
    ...
    synonymData <- merge(
      synonymDataFiltered[, "concept_id_2"], dataOntology[, c("concept_id", "domain_id", "vocabulary_id")],
      by.x = "concept_id_2", by.y = "concept_id"
    )
    colnames(synonymData) <- c("concept_id", "domain_id", "vocabulary_id")
    synonymCodes <- paste(
      union(dataCriteriaMapped$concept_id, synonymDataFiltered$concept_id_2),
      collapse = ","
    )  ## adds original codes into ancestor query (b/c of scenarios with ATC))
    ...
    conceptsCombined <- rbind(
      dataCriteriaMapped[, c("concept_id", "domain_id", "vocabulary_id")],
      synonymData
    )
  ...
  if (length(searchTable$Condition) >
    0) {
  ...
  if (length(searchTable$Observation) >
    0) {
  ...
  if (length(searchTable$Measurement) >
    0) {
  ...
  if (length(searchTable$Procedure) >
    0) {
  ...
  if (func_type == "or") {
    patient_list <- identifyPatientsOR(
      pts_condition, pts_observation, pts_measurement, pts_device, pts_drug,
      pts_procedure
    )
  } else if (func_type == "and") {
    patient_list <- identifyPatientsAND(
      dataCriteriaMapped, synonymDataFiltered, mappingDataInfo, pts_condition,
      pts_observation, pts_measurement, pts_device, pts_drug, pts_procedure
    )
  }
...
searchCondition <- function(useSource, codes) {
  conditionQuery <- paste0(
    "SELECT person_id, condition_concept_id FROM condition_occurrence WHERE condition",
    useSource, "_concept_id IN (", codes, ") "
  )
...
searchObservation <- function(useSource, codes) {
  observationQuery <- paste0(
    "SELECT person_id, observation_concept_id FROM observation WHERE observation",
    useSource, "_concept_id IN (", codes, ") "
  )
...
searchMeasurement <- function(useSource, codes) {
  measurementQuery <- paste0(
    "SELECT person_id, measurement_concept_id FROM measurement WHERE measurement",
    useSource, "_concept_id IN (", codes, ") "
  )
...
searchDrug <- function(useSource, codes) {
  drugQuery <- paste0(
    "SELECT person_id, drug_concept_id FROM drug_exposure WHERE drug", useSource,
...
searchDevice <- function(useSource, codes) {
  deviceQuery <- paste0(
    "SELECT person_id, device_concept_id FROM device_exposure WHERE device",
    useSource, "_concept_id IN (", codes, ") "
  )
...
searchProcedure <- function(useSource, codes) {
  procedureQuery <- paste0(
    "SELECT person_id, procedure_concept_id FROM procedure_occurrence WHERE procedure",
    useSource, "_concept_id IN (", codes, ") "
  )
